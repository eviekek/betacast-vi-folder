...
$filename = $sql->real_escape_string($_GET['filename']);
$ok = $sql->query("SELECT * FROM video WHERE mp4='$filename'");
$video = mysqli_fetch_assoc($ok);
$_SESSION['viewkey'] = random_int('1000', '100000');
?>
<head>
...
<body style="color:white; background-color:black;">
    ...
    <div class="video-stream" style="
        ...
        left: 0px;
    ">
    <video id="video-stream" onclick="playVid()" style="width:100%; height:100%;">
    </video>
...
function playVid() {
    ...
    if (!videoElement) {
        console.error("Error: Element with ID 'video-stream' not found.");
        return;
    }

    if (!videoElement.src) {
        videoElement.src = "/mp4files?id=<?php echo $video['id']; ?>&key=<?php echo $_SESSION['viewkey']; ?>";
    }
